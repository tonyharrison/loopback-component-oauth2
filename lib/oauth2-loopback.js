  ...
  function userLogin(username, password, done) {
    ...
    models.users.findByUsernameOrEmail(username, function(err, user) {
      ...
      user.hasPassword(password, function(err, matched) {
        ...
        if (options.userEmailVerificationRequired && !user.emailVerified) {
          debug('User email has not been verified');
          // Fail to log in if email verification is not done yet
          err = new TokenError(g.f('Credentials invalid as the email has not been verified'), 'invalid_grant');
          return done(err, false);
        }
        done(null, user);
      });
