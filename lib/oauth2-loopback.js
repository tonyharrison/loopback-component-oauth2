...
var url = require('url'),
  ...
  LocalStrategy = require('./strategy/loopback-realm').Strategy,
...
module.exports = function(app, options) {
  ...
  function userLogin(credentials, done) {
    debug('userLogin: %s realm: %s', credentials.username, credentials.realm);

    var UserModel = app.loopback.getModel(options.userModel);
    var normalizedCredentials = UserModel.normalizeCredentials(credentials,
        UserModel.settings.realmRequired,
        UserModel.settings.realmDelimiter);

    if (UserModel.settings.realmRequired && !normalizedCredentials.realm) {
      var err = new TokenError(g.f('Realm Required'), 'invalid_grant');
    ...
    var query = {
      where: {
        or: [
          ...
          {email: normalizedCredentials.username},
        ],
      },
    };

    if (UserModel.settings.realmRequired) {
      query.where.realm = normalizedCredentials.realm;
    }
    ...
    UserModel.findOne(query, function(err, user) {
      ...
      user.hasPassword(credentials.password, function(err, matched) {
        if (err || !matched) {
          return done(err, false);
        }
        done(null, user);
  ...
  if (supportedGrantTypes.indexOf('resourceOwnerPasswordCredentials') !== -1) {
    server.exchange(oauth2Provider.exchange.password(
      function(client, credentials, scope, done) {
        debug('Verifying username/password: %s %s %s',
          clientInfo(client), credentials.username, scope);
        ...
        userLogin(credentials, function(err, user) {
          if (err || !user) {
          ...
          debug('Generating access token: %j %s %s %s',
            token, clientInfo(client), credentials.username, scope);
