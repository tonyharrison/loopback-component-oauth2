...
module.exports = function(app, options) {
  ...
  function userLogin(credentials, done) {
    ...
    var nomalizedCredentials = UserModel.normalizeCredentials(credentials,
        UserModel.settings.realmRequired,
        UserModel.settings.realmDelimiter);

    if (UserModel.settings.realmRequired && !nomalizedCredentials.realm) {
    ...
    var query = {
      where: {
        or: [
          {username: nomalizedCredentials.username},
          {email: nomalizedCredentials.username},
        ],
      },
    };

    if (UserModel.settings.realmRequired) {
      query.where.realm = nomalizedCredentials.realm;
    }
